...
if (fs.existsSync(buildScriptPath)) {
  ...
  expect(buildScript.includes('-ffile-prefix-map='), 'F-Droid recipe build script must normalize native checkout paths');
  expect(buildScript.includes('GNU SHA-1 build-id'), 'F-Droid recipe build script must normalize native GNU build IDs');
}

if (fs.existsSync(sharedBuildScriptPath)) {
  const sharedBuildScript = fs.readFileSync(sharedBuildScriptPath, 'utf8');
  expect(sharedBuildScript.includes('scripts/prepare-fdroid-source-tree.sh'), 'shared F-Droid build script must remove bundled Expo local Maven repositories');
  expect(sharedBuildScript.includes('scripts/check-fdroid-android-dependencies.sh'), 'shared F-Droid build script must run the Android non-free dependency guard');
  expect(sharedBuildScript.includes('scripts/verify-fdroid-apk.sh'), 'shared F-Droid build script must verify the generated APK');
