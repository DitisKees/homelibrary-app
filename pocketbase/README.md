...
The Home Library backend schema is versioned in `pb_migrations/`. Do not configure the required collections only through the PocketBase dashboard; schema changes that the app depends on should be represented by migrations committed here.
...
6. Point `EXPO_PUBLIC_POCKETBASE_URL` in the app's `.env` file at this PocketBase instance.
